...
@foreach($templates as $template)
    <table style="width: 100%; border-collapse: collapse;">
        ...
        <tr>
            ...
            <td colspan="9">{{ $template->payer_name }}</td>
        ...
        {{-- Row 31 (Order's Row 3): SUMMA Header --}}
        <tr>
            <td colspan="10"></td>
            <td colspan="6">SUMMA</td>
        </tr>
        {{-- Row 32 (Order's Row 4): Date and Amount --}}
        <tr>
            <td colspan="6">{{ $template->currday ? (new DateTime($template->currday))->format('d.m.Y') : '' }}</td>
            <td colspan="4"></td>
            <td colspan="6">{{ number_format($template->sumpay, 2, ',', ' ') }}</td>
        </tr>
        {{-- Row 33 (Order's Row 5): Payer --}}
        <tr>
            <td colspan="2">Naqd pul topshiruvchi:</td>
            <td colspan="9">{{ $template->payer_name }}</td>
            <td colspan="5">Дебет</td>
        </tr>
        {{-- Row 34 (Order's Row 6): COA Account --}}
        <tr>
            <td colspan="10"></td>
            <td colspan="6">{{ $template->clacc }}</td>
        </tr>
        {{-- Row 35 (Order's Row 7): Bank Name --}}
        <tr>
            <td colspan="3">Qabul qiluvchi bank nomi</td>
            <td colspan="8">{{ $template->cashbox->label ?? '' }}</td>
            <td colspan="5"></td>
        </tr>
        {{-- Row 36 (Order's Row 8): Bank/BXM Codes --}}
        <tr>
            <td></td>
            <td>Bank kodi</td>
            <td colspan="2"></td>
        ...
        {{-- Row 37 (Order's Row 9): Payer Name and Account --}}
        <tr>
            <td colspan="2">Pul kirim qilinadigan xoʻjalik subyekti nomi va x/r</td>
            <td colspan="8">{{ $template->coname }}</td>
            <td colspan="6">{{ $template->coacc }}</td>
        </tr>
        {{-- Row 38 (Order's Row 10): Amount in Words --}}
        <tr>
            <td colspan="3">Summa soʻz bilan -</td>
            <td colspan="13">{{ $template->sumpay_in_words }}</td>
        </tr>
        {{-- Row 39 (Order's Row 11): Payment Purpose --}}
        <tr>
            <td colspan="2">To'lov maqsadi</td>
            <td colspan="14">{{ $template->paypurpose }}</td>
        </tr>
        {{-- Row 40 (Order's Row 12): Empty with underline --}}
        <tr>
            <td colspan="7"></td>
            <td colspan="9"></td>
        </tr>
        {{-- Row 41 & 42 (Order's Row 13 & 14): Signatures --}}
        <tr>
            <td colspan="2" rowspan="2">naqd pulni topshiruvchi shaxs imzosi:</td>
            <td colspan="3" rowspan="2"></td>
            <td colspan="2">Buxgalter:</td>
